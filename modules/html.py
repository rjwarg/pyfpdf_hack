...
# logger = logging.getLogger("web2py.app.report")
...
class HTML2FPDF(HTMLParser):
    ...
    def handle_charref(self, name):
        if name.startswith('x'):
            ...
            c = int(name)
        self.cell_text += chr(c)
        
    def handle_entityref(self,name):
        self.cell_text += entitydefs[name]
...
class HTMLMixin(object):
    def write_html(self, text, image_map=None):
        ...
        # logger.info("in write_html() prior to HTML2FPDF.feed() Text = %s",text)
        h2p.feed(text)
